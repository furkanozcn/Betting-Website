...
<div id="mid">
<form action="#" method="post" name="kusec">
<center><table class="tablo" width="515" height="400" border="7" cellpadding="2">
  <tr>
    <th colspan="3" scope="col">Kullanıcı Bakiye Ekle/Çıkar</th>
    </tr>
  <tr>
    <td width="147" align="center">Kullanıcı Seç:</td>
    <td width="110">
    <select name="kullanicisec" onChange="<?php 
if($_POST){
        echo "Kayıt silindi";
}else{
    ...
    ?>">
    <option hidden="" selected="selected" value="">Seçiniz..</option>
...
{	
	...
	$kisi=$oku['name'];
	$id=$oku['id'];	
	echo"<option value=\"".$para."\" >".$name."|".$kisi."</option>";
	if (isset($_POST['kullanicisec'])) {
                if($_POST["kullanicisec"] == $name){
                    echo "test";
                
	/*echo "<option value=\"kullanici=".$oku['username']."\" ".$selected.">".$oku['username']." | ".stripslashes($oku['name'])."</option>";*/
				}}
    ...
    <td width="218" align="center">Mevcut Bakiye:
	<?php 
if(isset($_POST['kusec'])){
$secili = $_POST['kusec']; 
echo $secili;
}
?>
TL</td>
  ...
  <tr>
    <td align="center">İşlem Türü:</td>
    <td colspan="2"><select name="islem" onChange="islemmenu"><option selected="selected">Seçiniz..</option>
<option value="ekle">(+)Bakiye Ekle</option>
<option value="cikar">(-)Bakiye Çıkar</option></select></td>
  </tr>
  <tr>
    <td align="center">Miktar:</td>
    <td colspan="2"><input name="a" type="text" size="8" maxlength="8" onBlur="check_login(this)">.<input name="miktar" type="text" size="3" maxlength="3"> TL</td>
  </tr>
  <tr>
